...
// MARK: Tensor view with floating point scalars

struct TensorFloatThumbnail<Scalar: TensorFlowScalar>: View where Scalar: BinaryFloatingPoint {
    /// Visualized tensor.
...
extension TensorSlice where Scalar: BinaryFloatingPoint {
    func normalized() -> [UInt8] where Scalar: BinaryFloatingPoint {
        let min = scalars.min()!
        let max = scalars.max()!
        let range = max - min
        
        if range.magnitude < Scalar.ulpOfOne {
            return Array(repeating: UInt8.max / 2, count: scalars.count)
        }

        return scalars.map { scalar in
            UInt8((scalar - min) / range * Scalar(UInt8.max))
        }
    }
