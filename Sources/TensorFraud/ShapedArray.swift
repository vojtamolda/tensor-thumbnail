...
extension ShapedArray: ExpressibleByArrayLiteral where Scalar: TensorFlowScalar {
  ...
  public init(arrayLiteral elements: _TensorElementLiteral<Scalar>...) {
    ...
    var shape = elements[0].tensor.shape
    shape.insert(elements.count, at: 0)

    var scalars: [Scalar] = []
    for element in elements {
        scalars.append(contentsOf: element.tensor.scalars)
    }
...
// FIXME: Disabled to prevent duplicate protocol conformance.
...
extension ShapedArray where Scalar: FixedWidthInteger {
    public init(randomUniform shape: [Int], lowerBound: Scalar = 0, upperBound: Scalar = 1) {
        let randomScalars = (0..<shape.reduce(1, *)).map { _ in
            Scalar.random(in: lowerBound...upperBound)
        }
        self.init(shape: shape, scalars: randomScalars)
    }
}

extension ShapedArray where Scalar == Float {
    public init(randomUniform shape: [Int], lowerBound: Scalar = 0, upperBound: Scalar = 1) {
        let randomScalars = (0..<shape.reduce(1, *)).map { _ in
            Scalar.random(in: lowerBound...upperBound)
        }
...
extension ShapedArray where Scalar == Double {
    public init(randomUniform shape: [Int], lowerBound: Scalar = 0, upperBound: Scalar = 1) {
        let randomScalars = (0..<shape.reduce(1, *)).map { _ in
            Scalar.random(in: lowerBound...upperBound)
        }
